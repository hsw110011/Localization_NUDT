...
int main(int argc, char **argv)
{
    ...
    std::ofstream csv_file(csv_dir + "/pose_compare_baseline.csv", std::ios::out | std::ios::trunc);
    if (!csv_file.is_open())
    {
    ...
    while (ros::ok())
    {
        ...
        else if(input.Odom_refreshflag == true && input.Inspvax_refreshflag == true && Init_localization == true)
        {
            ...
            //   T_{t-1}^{-1} * T_t, 避免经由 LocalToGlobal 绕行
            {
                const auto& q = input.Odom->pose.pose.orientation;
            ...
            // (5.2.4) 观测更新（基线）
            // 硬标签 mask 对比实验（baseline）
            if (input.BevMasks != nullptr)
            {
                cv_bridge::CvImagePtr cv_ptr;
                try
                {
                    // 必须使用 BGR8 (3通道) 与 TDF 对应
                    cv_ptr = cv_bridge::toCvCopy(*input.BevMasks, sensor_msgs::image_encodings::TYPE_8UC3);
                }
                ...
                if (cv_ptr && !cv_ptr->image.empty())
                {
                    // 检查是否只有0和1且是3通道
                    double min_v, max_v;
                    cv::minMaxLoc(cv_ptr->image, &min_v, &max_v);
                    int cn = cv_ptr->image.channels();
                    if (max_v > 1.0 + 1e-6)
                    {
                        ROS_WARN_THROTTLE(2.0, "[Data Check] Mask MaxValue=%.1f (Expect 1.0). Is it 0-255? Channels=%d", max_v, cn);
                    }

                    if (cn != 3)
                    {
                         ROS_WARN_THROTTLE(2.0, "[Data Check] Mask Channels=%d (Expect 3)", cn);
                    }

                    cv::Mat bev_mask_resized;
                    // 确保是 400x400
                    if (cv_ptr->image.rows != 400 || cv_ptr->image.cols != 400)
                    {
                        cv::resize(cv_ptr->image, bev_mask_resized, cv::Size(400, 400), 0, 0, cv::INTER_NEAREST);
                        std::cout << "[Info] 重新更改尺寸 ("
                                  << cv_ptr->image.cols << "x" << cv_ptr->image.rows
                                  << ") 到 400x400." << std::endl;
                    }
                    else
                    {
                        bev_mask_resized = cv_ptr->image;
                    }

                    cv::Mat vis_mask;
                    bev_mask_resized.convertTo(vis_mask, -1, 255.0);
                    int rect_w = 15;
                    int rect_h = 30;
                    cv::Rect center_rect(200 - rect_w/2, 200 - rect_h/2, rect_w, rect_h);
                    cv::rectangle(vis_mask, center_rect, cv::Scalar(255, 255, 255), 1);
                    cv::namedWindow("DEBUG: Recv Bev Mask (x255)", cv::WINDOW_NORMAL);
                    cv::resizeWindow("DEBUG: Recv Bev Mask (x255)", 400, 400);
                    cv::imshow("DEBUG: Recv Bev Mask (x255)", vis_mask);
                    cv::waitKey(1);

                    // 使用 3 通道的掩码更新粒子
                    std::cout << "====================*************************===================" <<std::endl;

                    pf.Update(bev_mask_resized, 20);

                    // 标准 SIR: 根据有效粒子数自适应重采样；估计/可视化后再执行。
                    should_resample = (pf.GetNeff() < 170.0);
                }
            }
        ...
        cv::Rect legend_rect(legend_x - 15, legend_y - 25, 700, 300);
