...
public:
    ...
    struct ObstacleRect {
        ...
        std::vector<ObstacleRect> near_obstacles_;
        std::vector<ObstacleRect> far_obstacles_;
...
private:
    ...
    // 障碍物检测函数
    void obstacle_detection(cv::Mat& vehicle_height_mat, cv::Mat& far_range_height_diff);

    // 可视化函数
    void visualizeCombinedObstacleDetection(const cv::Mat& near_height_mat, const cv::Mat& far_height_mat,
    ...
    struct ObstacleConfig {
        int size;           // 邻域尺寸 (3, 5, 7, 9, ...)
        int threshold;      // 最小邻居数阈值
        float real_size;    // 实际物理尺寸(米)
        std::string name;   // 配置名称
        cv::Scalar color;   // 可视化颜色 (B, G, R)
    };

    // 障碍物检测矩阵 - 预分配，避免重复分配内存
     
    cv::Mat obstacle_mat;
    cv::Mat used_mat;
    std::vector<cv::Mat> detection_mats;  // 动态检测结果矩阵
    std::vector<ObstacleConfig> obstacle_configs;  // 检测配置

    // 为兼容性保留的矩阵（指向detection_mats中对应的矩阵）
    cv::Mat neibor_5_mat;
    cv::Mat neibor_3_mat;

    // 远距离区域处理函数
    cv::Mat processFarRangePoints(const std::vector<PointXYZRGBValid>& colored_points);
